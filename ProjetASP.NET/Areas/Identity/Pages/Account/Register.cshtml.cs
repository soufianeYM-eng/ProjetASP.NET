...
namespace ProjetASP.NET.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if(!await _roleManager.RoleExistsAsync(SD.ProfessorEndUser))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(SD.ProfessorEndUser));
                    }
                    ...
                    if (User.IsInRole(SD.AdminEndUser))
                    {
                        ...
                        else if(Input.RoleIS == 1)
                        {
                            await _userManager.AddToRoleAsync(user, SD.StudentEndUser);
                        }
                        else
                        {
                            await _userManager.AddToRoleAsync(user, SD.ProfessorEndUser);
                        }
